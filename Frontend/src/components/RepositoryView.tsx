...
import type { Scheme as ApiScheme } from '../api/types';

interface RepositoryViewProps {
  onStartEligibility: () => void;
  onOpenSchemeModal: (scheme: Scheme) => void;
}
const mapApiSchemeToUiScheme = (scheme: ApiScheme): Scheme => ({
  id: scheme.schemeId,
  title: scheme.name,
  shortName: scheme.schemeId,
  ministry: scheme.ministry,
  category: scheme.category as Scheme['category'],
  sectorType: scheme.sectorType as Scheme['sectorType'],
  benefitHeadline: scheme.benefitHeadline,
  maxCeilingText: scheme.maxCeilingText,
  ...
  useEffect(() => {
    ...
    const loadSchemes = async () => {
      try {
        ...
        const response = await getSchemes(true);

        if (!isMounted) return;

        const mappedSchemes = response.data.map(mapApiSchemeToUiScheme);
  ...
  const filteredSchemes = useMemo(() => {
    ...
    return schemes.filter((s) => {
      ...
      const matchesSearch =
        ...
        s.ministry.toLowerCase().includes(normalizedSearch) ||
        s.description.toLowerCase().includes(normalizedSearch) ||
        s.benefitHeadline.toLowerCase().includes(normalizedSearch);
        ...
        <div className="flex flex-wrap items-center gap-2">
          {[
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer ${activeCategory === cat.id
